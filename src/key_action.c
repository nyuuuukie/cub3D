...
/*   Updated: 2021/03/09 22:21:08 by mhufflep         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		key_press(int keycode, t_all *all)
{
	if (!all->keys.w && keycode == KEY_W)
		all->keys.w = 1;
	if (!all->keys.s && keycode == KEY_S)
		all->keys.s = 1;
	if (!all->keys.a && keycode == KEY_A)
		all->keys.a = 1;
	if (!all->keys.d && keycode == KEY_D)
		all->keys.d = 1;
	if (!all->keys.l && keycode == KEY_LEFT)
		all->keys.l = 1;
	if (!all->keys.r && keycode == KEY_RIGHT)
		all->keys.r = 1;
	if (keycode == KEY_ESC)
		stop_engine(all);
	key_action(all);
