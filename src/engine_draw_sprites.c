...
/*   Updated: 2021/04/03 16:30:09 by mhufflep         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_sprite_line(t_all *all, int i, int sx)
{
	...
	while (sy < all->s_end.y)
	{
		...
		if (all->tex.x >= 0 && all->tex.x <= all->s->w &&
			...
			c = color_from_txt(all->s, all->tex.x, all->tex.y);
		else
			c = 0x00000000;
		if (all->map->bonus && all->keys.k0 && (c & 0x00FFFFFF) != 0)
			c = color_make_darker(all->sprites[i].p.dist / 40, c);
		if ((c & 0x00FFFFFF) != 0)
			put_pixel(&all->img, sx, sy, c);
		sy++;
	}
}
...
void	draw_sprite(t_all *all, int i)
{
	...
	{
		all->tex.x = 256 * (sx - (-all->s_size.x / 2 + all->sp_scr_x));
		all->tex.x = all->tex.x * all->s->w / all->s_size.x;
		all->tex.x = (int)all->tex.x / 256 + 1;
